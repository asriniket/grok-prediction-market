...
/**
 * Normalize X Activity's current post.mention.create event into the bot's
 * existing XPost shape. All other payloads are deliberately ignored.
 */
export function xActivityMentions(payload: unknown, botUserId: string): XPost[] {
  const root = object(payload);
  if (!root) return [];
  return activityMention(root, botUserId);
}
