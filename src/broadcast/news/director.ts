import { generateSpeakerClip } from './anchor.js';
 ...
 *
 * Only the opening beat of most segments goes on camera; the rest are
 * voice-over. That keeps video spend proportional to what it buys, and matches
 * how financial TV actually works — the presenter is off screen for most of the
 * airtime. Two-handers are the exception: every beat is on camera, because the
 * entire point is seeing both of them.
 */

export type NewsSegment =
  | { kind: 'beat'; at: number; line: string; speaker: string; treatment: string; kicker: string; segment: string; marketId?: string; videoUrl?: string; audioPath?: string; seconds: number }
...
const BOARD_HOLD_MS = 5000;
const sleep = (ms: number) => new Promise((r) => setTimeout(r, ms));

type PreparedBeat = { beat: Beat; media: Promise<{ videoUrl?: string; audioPath?: string; seconds: number }> };
...
export class NewsDirector {
  ...
  private async prepare(lookahead: number) {
    ...
    try {
      ...
      const onCam = fresh.filter((b) => b.onCamera).length;
      log(`prepared ${id} — ${fresh.length} beats (${onCam} on camera)`);

      this.queue.push({
        id,
        badge: SEGMENTS[id].badge,
        beats: fresh.map((beat) => ({
          beat,
          media: beat.onCamera
            ? generateSpeakerClip(beat.speaker, beat.line).then((c) => ({ videoUrl: c.videoUrl, seconds: c.seconds }))
  ...
  async run(): Promise<void> {
    ...
    while (!this.stopped) {
      ...
      if (swing && Math.abs(swing.to - swing.from) >= 0.08) {
      ...
      onSegment({ kind: 'sting', at: Date.now(), segment: seg.id, badge: seg.badge, holdMs: 1800 });
      await sleep(1800);
